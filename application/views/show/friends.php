...
<div class='box'>
    <?php
    if (count($friends) == 0) {
        show_message('No friends to show.', 'info');
    } else {
    ?>
        <div class='friends'>
...
<?php if ($has_next) { ?>
    <div class='box more'>
        <?php
        if ($is_visitor) {
            print "<a href='" . base_url("user/friends/{$suid}/{$next_offset}") . "'>Show more friends</a>";
        }
        else {
            print "<a href='" . base_url("user/friends/{$_SESSION['user_id']}/{$next_offset}") . "'>Show more friends</a>";
        }
        ?>
    </div>
<?php } ?>
