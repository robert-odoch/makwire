...
class Photo extends CI_Controller
{
    ...
    public function like($photo_id = 0)
    {
        if (is_ajax_request()) {
            try {
                $like_id = $this->photo_model->like($photo_id, $_SESSION['user_id']);
                $num_likes = $this->photo_model->get_num_likes($photo_id);
    ...
    public function comment($photo_id = 0)
    {
        ...
        try {
            ...
            if (is_ajax_request()) {
                $result['error'] = "Sorry, we couldn't find the photo.";
                echo json_encode($result);
                return;
            }
        ...
        if ( ! $this->user_model->are_friends($_SESSION['user_id'], $photo['user_id'])) {
            $message = "You don't have the proper permissions to comment on this photo.";
